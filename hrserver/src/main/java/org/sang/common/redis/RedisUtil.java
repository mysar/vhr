...
public class RedisUtil {
     ...
     * @return true成功 false失败
     */
    public boolean hmset(final String key, final Map<String,Object> map, long time){
        try {
            redisTemplate.opsForHash().putAll(key, map);
            if(time>0){
                expire(key, time);
    ...
    public boolean hset(final String key, final String item, final Object value) {
        try {
            ...
            logger.error("向一张hash表中放入数据异常：{}", e);
            return false;
        }
    }
     ...
     * @return
     */
    public double hincr(final String key, final String item, final double delta){
     ...
     * @return
     */
    public double hdecr(final String key, final String item, final double delta){
        return redisTemplate.opsForHash().increment(key, item,-delta);
    }
    ...
    public Set<Object> sGet(final String key){
        try {
            ...
            logger.error("根据key获取Set中的所有值异常：{}", e);
    ...
    public boolean sHasKey(final String key, final Object value){
        try {
            ...
            logger.error("根据value从一个set中查询,是否存在异常：{}", e);
            return false;
        }
    }
    ...
    public long setRemove(final String key, final Object ...values) {
        try {
            ...
            logger.error("移除值为value对应的数据异常：{}", e);
            return 0;
        }
    }
    ...
    public Object lGetIndex(final String key, final long index){
        try {
            ...
            logger.error("通过索引 获取list中的值异常：{}", e);
            return null;
        }
    }
    ...
    public long lRemove(final String key, final long count, final Object value) {
        try {
            ...
            logger.error("移除N个值为value对应的数据异常：{}", e);
            return 0;
        }
    }
}
